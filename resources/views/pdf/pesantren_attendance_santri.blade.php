...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .santri-card { background:#f0faf4; border:1px solid #c3e6cf; border-radius:6px; padding:10px 14px; margin-bottom:14px; display:flex; gap:16px; }
        .sc-label { font-size:8px; color:#888; text-transform:uppercase; }
        .sc-value { font-size:13px; font-weight:700; color:#1a6b3c; margin-top:1px; }
...
@if(!empty($stats))
<div class="stats-row">
    <div class="stat-box"><div class="stat-label">Total Hari</div><div class="stat-value" style="color:#555;">{{ $stats['total'] ?? 0 }}</div></div>
    ...
    <div class="stat-box"><div class="stat-label">Alpha</div><div class="stat-value" style="color:#c0392b;">{{ $stats['absent'] ?? 0 }}</div></div>
    ...
    <div class="stat-box">
        ...
        <div class="stat-value" style="color:#e6a817; font-size:11px;">{{ $stats['total_late_minutes'] ?? 0 }} mnt</div>
    </div>
</div>
@endif
    ...
    <tbody>
        @forelse($attendances as $i => $a)
            <tr>
                ...
                <td style="text-align:center; font-weight:600;">{{ $a->time_in ? substr($a->time_in, 0, 5) : '-' }}</td>
                <td style="text-align:center; font-weight:600;">{{ $a->time_out ? substr($a->time_out, 0, 5) : '-' }}</td>
                <td style="text-align:center; color:#888;">{{ $a->scheduled_in ? substr($a->scheduled_in, 0, 5) : '-' }}</td>
                <td style="text-align:center; color:#888;">{{ $a->scheduled_out ? substr($a->scheduled_out, 0, 5) : '-' }}</td>
        ...
        @empty
            <tr><td colspan="9" style="text-align:center; color:#aaa; font-style:italic; padding:16px;">Tidak ada data absensi.</td></tr>
        @endforelse
    </tbody>
</table>
 
<div class="footer">
    {{ $company->name ?? 'Pesantren' }} &mdash; Rekap Absensi &mdash; {{ $santri->name }} &mdash; {{ $generatedAt }}
</div>
</body>
</html>