...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .printed { margin-left:auto; font-size:9px; color:#aaa; text-align:right; }
 
        .santri-card { background:#f0faf4; border:1px solid #c3e6cf; border-radius:6px; padding:10px 14px; margin-bottom:14px; display:flex; gap:20px; align-items:center; }
        ...
        .ringkasan-section { margin-bottom:14px; }
        .section-title { font-size:11px; font-weight:700; color:#1a6b3c; margin-bottom:6px; border-left:3px solid #1a6b3c; padding-left:8px; }
        .ringkasan-grid { display:flex; gap:8px; flex-wrap:wrap; }
        .jilid-box { border:1px solid #c3e6cf; border-radius:5px; padding:6px 10px; min-width:80px; text-align:center; background:#fafff9; }
        .jilid-label { font-size:8px; color:#888; text-transform:uppercase; }
        .jilid-value { font-size:13px; font-weight:700; color:#1a6b3c; }
        .jilid-sub { font-size:8px; color:#888; }
        ...
        .nilai { display:inline-block; padding:2px 7px; border-radius:4px; font-size:9px; font-weight:700; }
        .n-Ap  { background:#e8f5ee; color:#155724; }
        .n-A   { background:#d4edda; color:#155724; }
        .n-Am  { background:#d4edda; color:#155724; }
        .n-Bp  { background:#cce5ff; color:#004085; }
        .n-B   { background:#cce5ff; color:#004085; }
        .n-Bm  { background:#fff3cd; color:#856404; }
        .n-Cp  { background:#fff3cd; color:#856404; }
        .n-C   { background:#ffe8cc; color:#7a3f00; }
        .n-Cm  { background:#f8d7da; color:#721c24; }
        .n-Dp  { background:#f8d7da; color:#721c24; }
        .n-D   { background:#f5c6cb; color:#721c24; }
        .n-Dm  { background:#f5c6cb; color:#721c24; }
 
        .lanjut-ya  { color:#1a6b3c; font-weight:700; }
        .lanjut-ulang { color:#c0392b; font-weight:700; }
        .paraf-ya   { color:#1a6b3c; }
        .paraf-no   { color:#aaa; }
    ...
    <div class="ringkasan-grid">
        @foreach($ringkasanJilid as $rj)
            <div class="jilid-box">
                ...
                <div class="jilid-sub" style="color:#1a6b3c; font-weight:600;">{{ $rj->total_lanjut }}× lanjut</div>
            </div>
        @endforeach
    </div>
</div>
        ...
        @forelse($records as $i => $r)
            ...
            <tr>
                ...
                <td style="font-size:9px; color:#555;">
                    ...
                    @if($r->signed_at)
                        <br><span style="font-size:8px; color:#aaa;">{{ \Carbon\Carbon::parse($r->signed_at)->format('d/m') }}</span>
                    @endif
                </td>
            </tr>
        @empty
...
<div class="footer">
    {{ $company->name ?? 'Pesantren' }} &mdash; Kartu Prestasi Ngaji &mdash; {{ $santri->name }} &mdash; {{ $generatedAt }}
</div>
</body>
</html>