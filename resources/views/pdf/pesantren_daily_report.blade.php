...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        body { font-family: sans-serif; font-size:10px; color:#222; background:#fff; padding:20px 24px; }
        .header { margin-bottom:14px; border-bottom:2px solid #1a6b3c; padding-bottom:10px; }
        .header-top { display:flex; align-items:center; gap:14px; }
        .logo { height:48px; width:auto; }
        .company-name { font-size:14px; font-weight:700; color:#1a6b3c; }
        .report-title { font-size:17px; font-weight:800; color:#111; margin-top:2px; }
        .report-sub { font-size:10px; color:#666; margin-top:1px; }
        .printed-at { margin-left:auto; font-size:9px; color:#aaa; text-align:right; white-space:nowrap; }
 
        .stats-row { display:flex; gap:8px; margin-bottom:14px; }
        .stat-box { flex:1; background:#f0faf4; border:1px solid #c3e6cf; border-radius:5px; padding:7px 10px; }
        .stat-label { font-size:8px; color:#666; text-transform:uppercase; letter-spacing:0.3px; }
        .stat-value { font-size:16px; font-weight:800; color:#1a6b3c; margin-top:1px; }
 
        .santri-block { margin-bottom:14px; border:1px solid #ddd; border-radius:5px; overflow:hidden; }
        .santri-header { background:#1a6b3c; color:#fff; padding:6px 10px; display:flex; align-items:center; gap:8px; }
        .santri-name { font-size:11px; font-weight:700; }
        .santri-meta { font-size:9px; opacity:0.8; }
        .santri-stats { display:flex; gap:0; background:#f9f9f9; border-bottom:1px solid #eee; }
        .santri-stat { flex:1; padding:5px 8px; border-right:1px solid #eee; text-align:center; }
        .santri-stat:last-child { border-right:none; }
        .santri-stat-label { font-size:8px; color:#888; }
        .santri-stat-value { font-size:13px; font-weight:700; color:#333; }
        .rate-good { color:#1a6b3c; }
        .rate-mid  { color:#e6a817; }
        .rate-bad  { color:#c0392b; }
 
        table { width:100%; border-collapse:collapse; font-size:9px; }
        th { background:#e8f5ee; color:#1a6b3c; font-weight:700; padding:5px 6px; text-align:left; border-bottom:1px solid #c3e6cf; }
            ...
            <tbody>
                @foreach($item['reports'] as $r)
                    <tr>
                        ...
                        <td>{{ $r->target }}</td>
                        <td>{{ $r->achievement ?? '<span style="color:#aaa;font-style:italic;">Belum diisi</span>' }}</td>
