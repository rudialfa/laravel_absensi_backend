...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .report-card { border:1px solid #ddd; border-radius:6px; overflow:hidden; margin-bottom:12px; }
        .rc-header { background:#1a6b3c; color:#fff; padding:7px 12px; display:flex; justify-content:space-between; align-items:center; }
        .rc-title { font-size:12px; font-weight:700; }
        .rc-meta { font-size:9px; opacity:.85; }
        .rc-body { padding:10px 12px; }
        .rc-row { display:flex; gap:8px; margin-bottom:6px; }
        .rc-col { flex:1; }
        .rc-field-label { font-size:8px; color:#888; text-transform:uppercase; margin-bottom:2px; }
        .rc-field-value { font-size:10px; color:#333; line-height:1.4; }
 
        .badge { display:inline-block; padding:2px 8px; border-radius:10px; font-size:8.5px; font-weight:600; }
...
<div class="santri-card">
    <div><div class="sc-label">Nama</div><div class="sc-value" style="font-size:14px;">{{ $santri->name }}</div></div>
    <div><div class="sc-label">Kamar</div><div class="sc-value">{{ $santri->position ?? '-' }}</div></div>
    <div><div class="sc-label">Kelas</div><div class="sc-value">{{ $santri->department ?? '-' }}</div></div>
    <div><div class="sc-label">Email</div><div class="sc-value" style="font-size:11px; color:#555;">{{ $santri->email ?? '-' }}</div></div>
</div>
 
<div class="stats-row">
    <div class="stat-box"><div class="stat-label">Total</div><div class="stat-value" style="color:#555;">{{ $stats['total'] }}</div></div>
    <div class="stat-box"><div class="stat-label">Approved</div><div class="stat-value" style="color:#1a6b3c;">{{ $stats['approved'] }}</div></div>
    <div class="stat-box"><div class="stat-label">Submitted</div><div class="stat-value" style="color:#0066cc;">{{ $stats['submitted'] }}</div></div>
    <div class="stat-box"><div class="stat-label">Ditolak</div><div class="stat-value" style="color:#c0392b;">{{ $stats['rejected'] }}</div></div>
    <div class="stat-box"><div class="stat-label">Draft</div><div class="stat-value" style="color:#888;">{{ $stats['draft'] }}</div></div>
    <div class="stat-box"><div class="stat-label">Rata-rata Skor</div><div class="stat-value" style="color:#1a6b3c;">{{ $stats['avg_score'] }}</div></div>
</div>
