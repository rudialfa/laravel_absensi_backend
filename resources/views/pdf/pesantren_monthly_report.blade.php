...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .green { color:#1a6b3c; } .yellow { color:#e6a817; } .red { color:#c0392b; } .grey { color:#555; }
...
{{-- STATS --}}
<div class="stats-row">
    <div class="stat-box"><div class="stat-label">Total</div><div class="stat-value grey">{{ $stats['total'] }}</div></div>
    <div class="stat-box"><div class="stat-label">Approved</div><div class="stat-value green">{{ $stats['approved'] }}</div></div>
    <div class="stat-box"><div class="stat-label">Submitted</div><div class="stat-value" style="color:#0066cc;">{{ $stats['submitted'] }}</div></div>
    <div class="stat-box"><div class="stat-label">Ditolak</div><div class="stat-value red">{{ $stats['rejected'] }}</div></div>
    <div class="stat-box"><div class="stat-label">Draft</div><div class="stat-value grey">{{ $stats['draft'] }}</div></div>
    <div class="stat-box"><div class="stat-label">Rata-rata Skor</div><div class="stat-value green">{{ $stats['avg_score'] }}</div></div>
</div>
...
<table>
    ...
    <tbody>
        @forelse($reports as $i => $r)
            <tr>
                ...
                <td style="max-width:100px; word-wrap:break-word; font-size:8.5px; color:#555;">
                    @if($r->problem)<b>M:</b> {{ Str::limit($r->problem, 40) }}<br>@endif
                    @if($r->solution)<b>S:</b> {{ Str::limit($r->solution, 40) }}@endif
                </td>
        ...
        @empty
            <tr><td colspan="10" style="text-align:center; color:#aaa; font-style:italic; padding:16px;">Tidak ada data laporan bulanan.</td></tr>
        @endforelse
    </tbody>
</table>
